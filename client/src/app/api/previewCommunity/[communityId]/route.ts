...
export async function GET(request: NextRequest, {params}: { params: { gameId: string } }) {
    ...
    const query = Prisma.sql`
        with "current_user" as (SELECT * FROM rank_communities r WHERE r."userId" = ${payload.userId as number} AND "communityId"=${community_to_load.communityId}), "rank_community" as (SELECT * FROM rank_communities where "communityId"=${community_to_load.communityId})

        SELECT final."userId", final.username, final."totalPoints", final.rank
        FROM (SELECT *
              FROM (SELECT * FROM "rank_community" r ORDER BY r.rank, r."createdAt" LIMIT 6) as "r"
              UNION ALL
              SELECT *
              FROM (SELECT *
                    ...
                    WHERE (r.rank = (SELECT rank FROM "current_user")
                      ...
                      AND r."userId" <> ${payload.userId as number}
              ...
              UNION ALL
              SELECT *
              FROM (SELECT * FROM "rank_community" h ORDER BY h.rank DESC, h."createdAt" DESC LIMIT 4) as "temp"
             ) as "final"
