...
export async function GET(request: NextRequest,{ params } : { params : {communityId: string } }) {
    ...
    const query = Prisma.sql`
        with "rank_community" as (SELECT * FROM rank_communities where "communityId" = ${user_to_get.communityId} AND "username" = ${queryParams})

        SELECT final."userId", final.username, final."totalPoints", final.rank
        FROM (SELECT *
              FROM (SELECT *
                    FROM "rank_community" r
                    WHERE username = ${queryParams}) as "r") as "final"
